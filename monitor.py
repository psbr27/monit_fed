...
#maximum number of clients supported by this app
num_of_clients = 10
...
client_list = {} 
event_list = []
client_id_mgr = clientidlib.clientID()
    ...
    def parse_status(data, gi, version):
        ...
        for line in data.splitlines():
            ...
            if client_section:
                ...
                #if key is found; don't update dictionary 
                if str(session['username']) in client_list: 
                  pass
                else:
...
def client_hb_manager():
    log.debug("+++ client_hb_manager thread (T1)+++")
    try:
...
#'esc_sn01': 1, 'esc_sn08': 0, 'esc_sn023': 3, 'esc_sn021': 2}
#{"sensorId":"fed_esc_08"
        new_esc = esc_id[4:7] + "_sn" + esc_id[8:11]
        log.debug(new_esc)

        if new_esc in client_list:
            index = client_list[new_esc]
            log.debug("[%s]Found client id of esc %d" %(new_esc, index))
            #set the event to true for the found client
            log.debug(event_list[index])
            event_list[index].set()


""" increment handler class """
class Counter(object):
    def __init__(self, start = 0):
        self.lock = threading.Lock()
        self.value = start
    def increment(self, count, idx):
...
""" client handling for heart beat """
def client_handling(e, idx, c):
    hb_count = 0
    g_count = 0
    log.debug("[%d]Created client handler for esc" %idx)
    while not e.isSet():
        event_is_set = e.wait(15)
        if event_is_set:
          hb_count = 0
          g_count = g_count + 1
          log.debug("[%d] Processing event >>" %idx)
          #increment the heart beat counter
          c.increment(g_count, idx)
          e.clear()
        else:
          #no heart beat section
          hb_count = hb_count + 1
          if hb_count == 3:
            log.warn(":: No heart beat for id %d raise a SNMP TRAP ::" %idx)
            hb_count = 0
            snmpA.trigger_trap("heart beat missed trap") 


""" Ping response handler """
def update_ping_response_in_db(response):
  for k,v in response.items():
    mysql_queries.mysql_query_update_like_esc_tbl(v, 0, k)

""" status update via email """
def status_update_email():
  log.debug("status_update_email invoked()")
  #send update every day 9am (below is GMT -5)
  while True:
    now = time.localtime()
    if now.tm_hour == 14:
      if now.tm_min == 00:
        if now.tm_sec == 00:
          log.info("Sent an daily morning update to Sarath P")
          send_e_mail.update_esc_via_email()
...
def main(**kwargs):
    ...
    #create T1 : client heart beat manager
    T1 = threading.Thread(name='hb-manager', target=client_hb_manager)
    threads.append(T1)
    T1.start()

    #status update
    T2 = threading.Thread(name='status-update', target=status_update_email)
    threads.append(T2)
    T2.start()
    ...
    for i in range(0,num_of_clients):
      e = threading.Event()
      event_list.append(e)
      t = threading.Thread(target=client_handling, args=(e,i,counter,))
      threads.append(t)
      t.setName(str(i) + "_esc_client")
      t.setDaemon(True)
      t.start()
      
    while True:
      long(count)
      log.info("========================  ::start iteration [%d] ===========================" %(count))
      monitor = OpenvpnMgmtInterface(cfg, **kwargs)
      f       = open("openvpn.html", "w")
      OpenvpnHtmlPrinter(cfg, monitor)
      f.close()
      shutil.move("openvpn.html", "/var/www/html/esc_testing/openvpn.html")
      log.info(p_res) 
      update_ping_response_in_db(p_res)
      p_res.clear()
         #move to /var/www/html/esc_testing directory for web access
      time.sleep(interval-2)
      log.info("======================== ::end iteration [%d] ===========================" %count)
      count = count + 1
...
if __name__ == '__main__':
  ...
  #Parser - P1 
  P1 = Process(target=client.parse_connections, args=(Q1,))
  P1.start()

  #Ping process P2
  P2 = Process(target=ping.ping_connections, args=(Q2,p_res, ))
  P2.start()

  #SNMP stat processing - P3
  P3 = Process(target=snmp.rest_connections, args=(Q3,port_no, ))
  P3.start()

  #Rest server - P4 
  P4 = Process(target=api.rest_server, args=(Q3,))
  P4.start()
